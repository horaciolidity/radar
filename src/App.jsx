...
import { Activity, LayoutGrid, List as ListIcon, Info, RefreshCcw, ShieldAlert, Cpu } from 'lucide-react';
...
function App() {
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      ...
      } catch (error) { console.error("Wallet failed", error); }
  ...
  const fetchContracts = async (append = false) => {
    try {
      ...
      let query = supabase.from('contracts').select('*').order('timestamp', { ascending: false }).range(offset, offset + LIMIT - 1);
      if (activeFilters.network !== 'all') query = query.eq('network', activeFilters.network);
      const { data, error } = await query;
      if (error) throw error;
      if (data) {
        const newContracts = data.map(mapDbToInternal);
        setContracts(prev => append ? [...prev, ...newContracts] : newContracts);
        setHasMore(data.length === LIMIT);
      }
      setIsLoading(false);
      setIsFetchingMore(false);
    } catch (err) { console.error("Error fetching", err); setIsLoading(false); }
  };

  useEffect(() => {
    fetchContracts();
    const channel = supabase.channel('public:contracts').on('postgres_changes', { event: '*', schema: 'public', table: 'contracts' }, (payload) => {
  ...
  useEffect(() => {
    ...
    activeNetworks.forEach(network => {
      if (!intervalsRef.current[network]) {
        ...
        intervalsRef.current[network] = setInterval(() => { contractManager.scanRecentBlocks(network); }, 15000);
      }
    });
  }, [scanStatus.activeScans]);

  const toggleScanning = (networkName) => {
    setScanStatus(prev => ({ ...prev, activeScans: { ...prev.activeScans, [networkName]: !prev.activeScans[networkName] } }));
  };
          ...
          <div className="animate-in fade-in duration-500">
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-2">
                  <div className="flex items-center gap-2 text-primary font-mono text-sm uppercase">
                    ...
                    Hybrid Detection Mode Active {foundThisSession > 0 && `• [${foundThisSession} NEW HITS]`}
                  </div>
                  <h2 className="text-4xl font-black tracking-tight text-white uppercase italic">
                    Hybrid <span className="text-primary">Radar</span>
                  </h2>
                  <p className="text-zinc-400 max-w-xl">
                    Scanning for all market tokens and highlighting surgical vulnerabilities.
                    The scanner detects every new ERC20 and flags high-yield security risks in real-time.
                  </p>
                </div>
            ...
            <div className="flex items-center justify-between mb-6 px-2">
              ...
              <div className="flex items-center gap-4 text-[10px] text-zinc-500">
                ...
                <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-warning" /> Vulnerable</div>
                <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-danger" /> Critical</div>
              </div>
            </div>
            ...
            {filteredContracts.length > 0 ? (
              <motion.div layout className={viewMode === 'grid' ? "radar-grid" : "space-y-4 max-w-4xl mx-auto"}>
                <AnimatePresence mode='popLayout'>
                  {filteredContracts.map((contract) => (
                    <motion.div key={contract.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }}>
              ...
              <div className="py-24 text-center glass-card rounded-3xl border-dashed border-2 flex flex-col items-center gap-4 border-white/5">
                <div className="w-16 h-16 bg-surface-lighter rounded-full flex items-center justify-center text-zinc-600"><Cpu size={32} className="animate-spin-slow" /></div>
                <div className="space-y-1">
                  <h3 className="text-xl font-bold text-white">No signals detected</h3>
                  <p className="text-zinc-500">Scanning networks for tokens and vulnerabilities...</p>
                </div>
              </div>
            )}
          </div>
      ...
      <footer className="py-8 border-t border-white/5 mt-12 bg-surface/30">
        <div className="container mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest">
            &copy; 2026 Hybrid Radar Alpha // Professional Security Intel
          </div>
        </div>
      </footer>
    </div>
